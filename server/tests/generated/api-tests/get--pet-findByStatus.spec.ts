...
test.describe('GET /pet/findByStatus', () => {
  ...
  test('should Finds Pets by status', async () => {
    await allure.epic('API Testing');
    await allure.feature('GET /pet/findByStatus');
    await allure.story('/pet/findByStatus');
    await allure.description('Multiple status values can be provided with comma separated strings');
    await allure.tag('pet');
    
    // Prepare request details
    const requestDetails = {
      method: 'GET',
      url: 'https://fakerestapi.azurewebsites.net/pet/findByStatus',
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json'
      },
